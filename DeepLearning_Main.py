...
# ==========================================
def prepare_dataloaders(df, sampler_flag, model_name, batches=32, workers=0):
    print("\nPreparing DataLoaders...")
    IMAGENET_MEAN = [0.485, 0.456, 0.406]
    IMAGENET_STD = [0.229, 0.224, 0.225]
    SIZE = [299, 299] if model_name == 'inception' else [224, 224]
    ...
    train_benign    = train_df[train_df['label'] == 0].shape[0]
    train_malignant = train_df[train_df['label'] == 1].shape[0]

    majority_count      = int(max(train_benign, train_malignant))
    balanced_epoch_size = majority_count * 2

    weight_benign    = majority_count / train_benign
    weight_malignant = majority_count / train_malignant

    sampler_weights = torch.DoubleTensor(
        [weight_malignant if label == 1 else weight_benign for label in train_df['label']]
    )
        ...
        for inputs, labels in tqdm(test_loader, desc="Testing", leave=False):
            ...
            with torch.no_grad(), torch.autocast(device_type=device.type, enabled=(device.type == 'cuda')):
                if TTA_flag:
                    ...
                    stacked_inputs  = torch.cat([inputs, img_hflip, img_vflip, img_rot90], dim=0)
                    stacked_outputs = model(stacked_inputs)
                    out_orig, out_hflip, out_vflip, out_rot90 = stacked_outputs.chunk(4, dim=0)
                    test_loss += criterion(out_orig, labels).item() * inputs.size(0)
...
def print_table(title, args_dict):
    ...
    for key, val in args_dict.items():
        ...
        val_str = str(f"{val:.4f}") if key == "positive_weight" else val_str
...
def Run_Model(Data, device, model_name, pretrained_model_path, save_path):
    ...
    train_loader, val_loader, test_loader = prepare_dataloaders(df, sampler_flag, model_name, batches, workers)
