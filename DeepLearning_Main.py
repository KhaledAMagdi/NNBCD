...
# def execute_ensemble(data, device, ensemble_dict):
#     df = data.copy()
#     model_probs_dict = {}
#     master_labels = []
#
#     for model_name, weights_path in ensemble_dict.items():
#         print(f"\n-> Extracting Predictions for {model_name.upper()}")
...
#                 out_vflip = model(torch.flip(inputs, dims=[2]))
#                 out_rot90 = model(torch.rot90(inputs, k=1, dims=[2, 3]))
#                 probs = (torch.sigmoid(out_orig ) +
#                          torch.sigmoid(out_hflip) +
#                          torch.sigmoid(out_vflip) +
#                          torch.sigmoid(out_rot90)) / 4.0
#             current_model_probs.extend(probs.cpu().numpy())
#             if model_name == list(ensemble_dict.keys())[0]:
#                 master_labels.extend(labels.cpu().numpy())
#
#         model_probs_dict[model_name] = np.array(current_model_probs).squeeze()
#         del model
#         torch.cuda.empty_cache()
#         gc.collect()
#
#     master_labels = np.array(master_labels).squeeze()
#
#     print("\n--- Calculating Dynamic Ensemble Weights ---")
#     model_aucs = []
#
#     for model_name, safe_probs in model_probs_dict.items():
#         fpr, tpr, _ = skm.roc_curve(master_labels, safe_probs)
#         model_auc = skm.auc(fpr, tpr)
#         model_aucs.append(model_auc)
#         print(f"{model_name.upper()} Standalone AUC: {model_auc:.4f}")
#
#     aucs_array = np.array(model_aucs)
#     adjusted_scores = np.maximum(aucs_array - 0.5, 0.0001) ** 6
#     dynamic_weights = adjusted_scores / np.sum(adjusted_scores)
#     print(f"Dynamically Assigned Weights: {np.round(dynamic_weights, 3)}")
#     stacked_probs = np.stack(list(model_probs_dict.values()))
#     ensemble_probs = np.average(stacked_probs, axis=0, weights=dynamic_weights)
#     print("\n--- Optimizing Threshold for Patient Safety (Max F2) ---")
#     precision, recall, thresholds = skm.precision_recall_curve(master_labels, ensemble_probs)
...
#     print(f"Optimal F2 Threshold: {optimal_threshold:.4f}")
#     final_predictions = (ensemble_probs >= optimal_threshold).astype(int)
#     acc = accuracy_score(master_labels, final_predictions)
#     auc = roc_auc_score(master_labels, ensemble_probs)
#     f1 = f1_score(master_labels, final_predictions, zero_division=0)
#     recall = recall_score(master_labels, final_predictions, zero_division=0)
#     tn, fp, fn, tp = confusion_matrix(master_labels, final_predictions).ravel()
#     TE = fn + fp
#     TC = tp + tn
#     TI = TE + TC
#
#     print(f"Metrics  | AUCROC: {auc:.4f} | F-1: {f1:.4f}         | Threshold: {optimal_threshold:.2f}  ")
#     print(f"Score    | Recall: {recall:.4f} | Acc: {acc:.4f}         | Total Images: {TI}")
#     print(f"Detected | TP:      {tp:<4}  | TN:   {tn:<3}           | Total Clears: {TC}")
#     print(f"Errors   | FN:      {fn:<4}  | FP:   {fp:<3}           | Total Errors: {TE}")