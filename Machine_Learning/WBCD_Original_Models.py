...
def prepare_data(data, target_col):
    ...
    # Map labels : 2 (Benign) -> 0, 4 (Malignant) -> 1
    data[target_col] = data[target_col].astype(int).map({2: 0, 4: 1})
...
# ==========================================
def WBCD_Original_Analysis(data, target_col="Class"):
    lock_seeds()
    ...
    print("\n--- Final Model Comparison (Original) ---")
...
def Cross_Validate_Original(data, target_col="Class", n_splits=5):
    print(f"\nRunning {n_splits}-Fold Cross Validation...")

    df = data.copy()
    df.replace('?', np.nan, inplace=True)
    df.dropna(inplace=True)
    df[target_col] = df[target_col].astype(int).map({2: 0, 4: 1})
